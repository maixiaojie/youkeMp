  ...
  <view class="uni-swipe-action">
    ...
    >
      <view class="uni-swipe-action__content">
        ...
        <view class="row">
            <view class="left">
              <image :src="datas.icon"></image>
            </view>
            <view class="center">
              <view class="title">{{datas.title}}</view>
              <view class="info">{{datas.content}}</view>
            </view>
            <view class="right">
              <image src="/static/task/task_uncompleted.png"></image>
            </view>
          </view>
      </view>
      <view :id="elId" class="uni-swipe-action__btn-group">
        <div
          ...
          :key="index"
          :style="{backgroundColor: item.style && item.style.backgroundColor ? item.style.backgroundColor : '#C7C6CD',color: item.style && item.style.color ? item.style.color : '#FFFFFF',fontSize: item.style && item.style.fontSize ? item.style.fontSize : '28rpx'}"
...
export default {
  ...
  data() {
     ...
     */
    const elId = this.__call_hook
      ? "uni_swipe_action"
      : `Uni_${Math.ceil(Math.random() * 10e5).toString(36)}`;
    return {
      elId: elId,
      isShowBtn: false,
